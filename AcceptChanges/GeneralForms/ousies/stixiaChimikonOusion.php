...
<script>
    ...
    $(document).ready(function() {
        ...
        <div class="container">
                ...
                ?>
                            <?php 
                         $_SESSION["chemical_Name_".$k]=$row3["Name"]; 
                            $_SESSION["chemical_CAS_".$k]=$row3["Cas"];
                            $_SESSION["chemical_EINECS_".$k]=$row3["Ec"];
                            $_SESSION["chemical_IUPAC_".$k]=$row3["Iupac"];
                            $_SESSION["chemical_otherName_".$k]=$row3["Other_Name"];
                            $_SESSION["chemical_enarmonismeni_".$k]=$row3["Sort"];
                            $_SESSION["chemical_weight_".$k]=$row3["Quantity"];
                            $_SESSION["chemical_weight_type_".$k]=$row3["QuantityType"];
                            $_SESSION["convertP_".$k]=$row3["CLPp"];
                            $_SESSION["convertH_".$k]=$row3["CLPh"];
                            $_SESSION["iconsChem_".$k]=$row3["CLPicon"];

                            ?>
                <!--data-toggle="modal" data-target="#suplCompanyModal"-->
                <tr id="chemicaltexts<?php echo $k?>" data-toggle="modal" data-target="#sub" class="click" >
                    <td class="d0"><strong><?php echo $k ?></strong></td>
                        <td class="d1"><?php echo $row3["Name"]?></td>
                        <td class="d2"><?php echo $row3["Cas"]?></td>
                        <td class="d2"><input type="image" src="../../images/Deep_Edit.png" data-toggle="modal" onclick="show(this);" data-target="#chemicalModal"></td>
                        <td class="d3"><input type="image" src="../../images/delete-icon.png" onclick= "removeElementID(<?php echo $k;?>);"></td>
                </tr>
                <?php
                    $k=$k+1;
                   $row3 = mysql_fetch_array($sql3);
